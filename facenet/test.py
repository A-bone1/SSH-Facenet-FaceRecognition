...
def load_and_align_images(im, margin):
    aligned_images = []

    img=im = cv2.imdecode(np.fromstring(im, np.uint8), cv2.IMREAD_COLOR)
...
def calc_embs(im,margin=10, batch_size=1):
    aligned_images = prewhiten(load_and_align_images(im,margin))
    pd = []
...
def distanceResult(im1,im2):
    a1=calc_embs(im1, margin=10, batch_size=1)
    a2=calc_embs(im2, margin=10, batch_size=1)
